...
同样的额度错误也会以普通 HTTP 错误正文返回（例如直接 `429` 加 `{"error":{"message":"Error 429: Daily free limit reached ..."}}`）。两种形式都会读取正文里的恢复时间并记录冷却窗口。
...
渠道列表里 Cline 渠道的「账户信息」按钮打开额度对话框，显示账号邮箱、订阅状态、凭证到期时间，以及每个模型当前的额度状态：

- 冷却中的模型显示重置于的绝对时间和剩余时间，附上触发冷却的上游错误原文。
- 没有冷却记录的模型显示为可用。Cline 不提供查询免费额度的接口，成功响应的响应头里也没有额度信息，所以可用状态来自「没有失败记录」，不是实时余额。

对话框里的「检测额度」按钮对单个模型发送一条最长 1 token 的流式请求，把上游的真实结果写回显示，并刷新冷却记录。这个检测会消耗该模型的一次免费调用，所以只在需要确认时手动点击。

接口：

- `GET /api/channel/:id/cline/quota` 读取账号、凭证和当前冷却窗口，不消耗上游额度。
- `POST /api/channel/:id/cline/quota/probe`，请求体 `{"model":"kimi-k3"}`，发送一次最小请求并返回模型状态。
