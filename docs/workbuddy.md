...
只有真正转发出去的正文或带名字的调用才算答复。整条流一个可用答复都没有时，中转会写出一帧 `upstream stream ended without an answer`，并把保留的分片和最后一帧写进日志（`workbuddy relay stream: answers=… forwarded=… calls=… held=… last=…`），便于对照客户端报的断流。
